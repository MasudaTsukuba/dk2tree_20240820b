 ...
 */
LTree::LTree(LTree *left, LTree *right) {
    isLeaf = false;
 ...
 */
LTree::LTree(unsigned long size) {
    isLeaf = true;
    node = Node(size); 
}
...
LInternalNode::Entry LTree::findLeaf(unsigned long n, std::vector<LNesbo> *path) {
    if (path == nullptr) {
        auto *current = this;
        unsigned long bitsBefore = 0;
...
LInternalNode::Entry LTree::findLeaf2(unsigned long n, std::vector<LNesbo> &path) {
...
LTree *LTree::insertBlock(long unsigned index, std::vector<LNesbo> *path) {
    return this->insertBits(index, BLOCK_SIZE, path);
}

LTree *LTree::deleteBlock(long unsigned index, std::vector<LNesbo> *path) {
...
LLeafNode::LLeafNode(unsigned long size){
    // bv = BitVector(size);
 ...
 */
LLeafNode::LLeafNode(BitVector bv){
    // bv = bv; 
    this->bv = bv; //// 2024/8/31
}
